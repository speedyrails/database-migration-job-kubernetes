...
#
# Usage:
#   ./db-migration-job.sh "OCI_IMAGE:TAG" "K8s SECRET NAME LIST SEPARATED BY SPACES" "COMMAND_ENTRYPOINT" "COMMAND_ARGUMENT"
#
# Example:
#   db-migration-job.sh "myapp:latest" "cm1 cm2 cm3" "/cnb/lifecycle/launcher" "bundle exec rake db:migrate" "default" "300"
# 
...
MIGRATION_TIMEOUT="${6:-"1200"}"

## Check arguments
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ] || [ -z "$4" ] || [ -z "$5" ]; then
    echo "$(basename "$0"): Missing arguments or not arguments passed."
    echo "Usage:"
    echo "  ./db-migration-job.sh \"OCI_IMAGE:TAG\" \"K8s SECRET NAME LIST SEPARATED BY SPACES\" \"COMMAND_ENTRYPOINT\" \"COMMAND_ARGUMENT\" \"NAMESPACE\" \"MIGRATION_TIMEOUT_SECONDS\""
    echo "Example:"
    echo "  ./db-migration-job.sh \"myapp:latest\" \"cm1 cm2 cm3\" \"/bin/bash\" \"bundle exec rake db:migrate\" \"default\" \"300\""
    exit 1
else
    IMAGE_WITH_TAG="$1"
    SECRET_NAME_LIST="$2"
...
# Add the SECRETs to the job template
for SECRET_NAME in ${SECRET_NAME_LIST}; do
    echo "        - secretRef:" >> db-migration-job.yaml
    echo "            name: $SECRET_NAME" >> db-migration-job.yaml
done
...
kubectl -n $NAMESPACE wait --for=condition=complete --timeout="${MIGRATION_TIMEOUT}"s job/"${JOB_NAME}"

# Get the data migration job logs
DATA_MIGRATION_POD_ID=$(kubectl -n ${NAMESPACE} get pods --no-headers -o custom-columns=":metadata.name" -l job-name="${JOB_NAME}")
echo "### Data migration ouput logs ###"
kubectl -n $NAMESPACE logs "$DATA_MIGRATION_POD_ID" --since=1h

# Check if the job (pod) has succeeded
JOB_STATUS=$(kubectl -n $NAMESPACE get job "${JOB_NAME}" --no-headers -o custom-columns=":status.conditions[0].type")
