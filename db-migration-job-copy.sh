...
if ! kubectl wait --for=condition=complete --timeout="${MIGRATION_TIMEOUT}"s job/"${JOB_NAME}"; then
    echo "ERROR: The database migration job has timed out after ${MIGRATION_TIMEOUT} seconds."
